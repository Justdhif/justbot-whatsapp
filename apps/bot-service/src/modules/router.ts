...
export function isJuliaQuery(text: string): boolean {
  ...
  return (
    lower.includes("jujul") ||
    lower.includes("julia") ||
    lower.includes("irya") ||
    lower.includes("salsabillah")
  );
}

export async function handleJuliaSpecialResponse(
  userPrompt: string,
): Promise<string> {
...
export const MODULE_DETAILS: Record<
  string,
  { name: string; icon: string; desc: string; capabilities: string[] }
> = {
  coding: {
...
export async function processIncomingMessage(
  ...
  const isCodingQuery =
    /\b(code|coding|program|function|compile|debug|error|typescript|javascript|python|html|css|json|refactor|database|git)\b/i.test(
      lower,
  ...
  const isCreatorQuery =
    /\b(tiktok|reels|shorts|konten|creator|hook|script|naskah|fyps|hashtag|caption|video)\b/i.test(
      lower,
    );
  if (isCreatorQuery) {
  ...
  const isTranslatorQuery =
    /\b(terjemahkan|translate|artinya|inggris|jepang|mandarin|bahasa|polyglot)\b/i.test(
      lower,
    );
  ...
  const isOcrQuery = /\b(ocr|scan|ekstrak|baca gambar|salin tulisan)\b/i.test(
    lower,
  );
  if (isOcrQuery) {
  ...
  const isPdfQuery =
    /\b(pdf|dokumen|rangkum|baca berkas|summary|ringkas)\b/i.test(lower);
  if (isPdfQuery) {
    return await handlePdfAiModule(trimmed);
  }

  const isEmailQuery =
    /\b(email|surel|surat resmi|draf surat|izin cuti|lamaran kerja|formal draft)\b/i.test(
      lower,
    );
  if (isEmailQuery) {
  ...
  const isReminderQuery =
    /\b(reminder|jadwal|agenda|todo|catatan kerja|tugas hari ini|prioritas)\b/i.test(
      lower,
    );
  if (isReminderQuery) {
  ...
  const isUtilityQuery =
    ...
    /\b(konversi|hitung|berapa|kurangi|tambah|kali|bagi|kilo|mil|celcius|fahrenheit|usd|rupiah|idr|kurs|meter|cm|luas|volume|ibukota|tanggal|sejarah)\b/i.test(
      lower,
    );
  if (isUtilityQuery) {
  ...
  const namePrompt = senderName
    ? `Nama profil WhatsApp pengguna ini adalah "${senderName}". Sapa atau panggil namanya bila sesuai agar percakapan terasa personal.`
    : "";
